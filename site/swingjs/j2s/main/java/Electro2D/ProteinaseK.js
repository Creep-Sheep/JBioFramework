(function(){var P$=Clazz.newPackage("main.java.Electro2D"),I$=[];
/*c*/var C$=Clazz.newClass(P$, "ProteinaseK", null, 'main.java.Electro2D.Protease');

C$.$clinit$=2;

Clazz.newMeth(C$, '$init$', function () {
},1);

Clazz.newMeth(C$, 'c$',  function () {
;C$.superclazz.c$$CA$Z.apply(this,[Clazz.array(Character.TYPE, -1, ["A", "F", "I", "L", "V", "W", "Y"]), false]);C$.$init$.apply(this);
}, 1);
})();
;Clazz.setTVer('3.3.1-v4');//Created 2022-04-21 17:00:07 Java2ScriptVisitor version 3.3.1-v4 net.sf.j2s.core.jar version 3.3.1-v4
