...
var isTar=$I$(3).isTar$java_io_BufferedInputStream(bis);
...
var bytes=(ze == null  ? null : $I$(3,"getLimitedStreamBytes$java_io_InputStream$J",[zis, ze.getSize$()]));
ze=null;
zis.close$();
if (bytes == null ) return "";
if ($I$(3).isZipB$BA(bytes) || $I$(3).isPngZipB$BA(bytes) ) return C$.getZipFileDirectory$java_io_BufferedInputStream$SA$I$Z($I$(3).getBIS$BA(bytes), list, ++listPtr, asBufferedInputStream);
if (asBufferedInputStream) return $I$(3).getBIS$BA(bytes);
if (asBinaryString) {
ret=Clazz.new_($I$(5,1));
for (var i=0; i < bytes.length; i++) ret.append$S(Integer.toHexString$I(bytes[i] & 255)).appendC$C(" ");

return ret.toString();
}if ($I$(3).isGzipB$BA(bytes)) bytes=$I$(3,"getLimitedStreamBytes$java_io_InputStream$J",[C$.getUnGzippedInputStream$BA(bytes), -1]);
return $I$(3).fixUTF$BA(bytes);
} catch (e) {
...
bis.close$();
return (bytes == null  ? "" : asBufferedInputStream ? $I$(3).getBIS$BA(bytes) : $I$(3).fixUTF$BA(bytes));
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
return "";
} else {
...
try {
if ($I$(3).isTar$java_io_BufferedInputStream(bis)) return C$.getTarContents$java_io_BufferedInputStream$S$javajs_util_SB(bis, fileName, null);
bis=C$.getPngZipStream$java_io_BufferedInputStream$Z(bis, true);
var zis=Clazz.new_($I$(4,1).c$$java_io_InputStream,[bis]);
var ze;
while ((ze=zis.getNextEntry$()) != null ){
if (!fileName.equals$O(ze.getName$())) continue;
var bytes=$I$(3,"getLimitedStreamBytes$java_io_InputStream$J",[zis, ze.getSize$()]);
return (($I$(3).isZipB$BA(bytes) || $I$(3).isPngZipB$BA(bytes) ) && ++listPtr < list.length  ? C$.getZipFileContentsAsBytes$java_io_BufferedInputStream$SA$I($I$(3).getBIS$BA(bytes), list, listPtr) : bytes);
}
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
} else {
...
return null;
}var bytes=$I$(3).getLimitedStreamBytes$java_io_InputStream$J(bis, nbytes);
if (cache != null ) {
cache.put$O$O(name, Clazz.new_($I$(6,1).c$$BA,[bytes]));
...
return $I$(3,"fixUTF$BA",[$I$(3).getLimitedStreamBytes$java_io_InputStream$J(is, -1)]);
}, 1);
...
try {
return C$.getUnzippedInputStream$java_io_BufferedInputStream($I$(3).getBIS$BA(bytes));
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
return null;
} else {
...
while ($I$(3).isGzipS$java_io_InputStream(bis))bis=Clazz.new_([C$.newGZIPInputStream$java_io_InputStream(bis)],$I$(2,1).c$$java_io_InputStream);

return bis;
}, 1);

Clazz.newMeth(C$, 'getUnzippedInputStreamBZip2$java_io_BufferedInputStream',  function (bis) {
while ($I$(3).isBZip2S$java_io_InputStream(bis))bis=Clazz.new_([C$.newBZip2InputStream$java_io_InputStream(bis)],$I$(2,1).c$$java_io_InputStream);
...
bis.mark$I(75);
try {
var data=$I$(3).getLimitedStreamBytes$java_io_InputStream$J(bis, 74);
bis.reset$();
var pt=0;
...
if (!$I$(3).isPngZipStream$java_io_InputStream(bis)) return bis;
...
data=$I$(3).getLimitedStreamBytes$java_io_InputStream$J(bis, pt_count[1]);
}} catch (e) {
} finally {
try {
...
try {
var isZip=false;
if ($I$(3).isPngZipStream$java_io_InputStream(bis)) {
var isImage="_IMAGE_".equals$O(name);
if (name == null  || isImage ) bdata.put$O$O((isImage ? "_DATA_" : "_IMAGE_"), Clazz.new_([C$.getPngImageBytes$java_io_BufferedInputStream(bis)],$I$(6,1).c$$BA));
isZip=!isImage;
} else if ($I$(3).isZipS$java_io_InputStream(bis)) {
isZip=true;
} else if ($I$(3).isTar$java_io_BufferedInputStream(bis)) {
C$.cacheTarContentsStatic$java_io_BufferedInputStream$S$java_util_Map(bis, name, bdata);
} else if (name == null ) {
bdata.put$O$O("_DATA_", Clazz.new_([$I$(3).getLimitedStreamBytes$java_io_InputStream$J(bis, -1)],$I$(6,1).c$$BA));
} else {
...
var bytes=$I$(3).getLimitedStreamBytes$java_io_InputStream$J(zis, nBytes);
if (zipEntryRoot != null ) {
C$.readFileAsMapStatic$java_io_BufferedInputStream$java_util_Map$S($I$(3).getBIS$BA(bytes), cache, zipEntryRoot);
return null;
}(n=Long.$add(n,(bytes.length)));
var o=(asByteArray ? Clazz.new_($I$(6,1).c$$BA,[bytes]) : bytes);
...
var pt_count=Clazz.array(Integer.TYPE, [2]);
C$.getPngZipPointAndCount$java_io_BufferedInputStream$IA(bis, pt_count);
if (pt_count[1] != 0) return C$.deActivatePngZipB$BA($I$(3).getLimitedStreamBytes$java_io_InputStream$J(bis, pt_count[0]));
}return $I$(3).getLimitedStreamBytes$java_io_InputStream$J(bis, -1);
} catch (e) {
if (Clazz.exceptionOf(e,"java.io.IOException")){
return null;
} else {
...
if ($I$(3).isPngZipB$BA(bytes)) bytes[51]=(32|0);
return bytes;
}, 1);
