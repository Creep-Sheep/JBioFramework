...
if (icon == null  ? this.currentIcon != null  : !icon.equals$O(this.currentIcon)) this.setIconAndText$S$javax_swing_Icon$I$S("icon", icon, this.currentGap, this.currentText);
}});
...
if (itop >= 0) {
C$.addJSKeyVal$O$SA(cssTxt, Clazz.array(String, -1, ["top", top + "%", "transform", "translateY(" + (yoff == null  ? "-" + top + "%"  : yoff) + ")" + voff ]));
...
C$.addJSKeyVal$O$SA(cssIcon, Clazz.array(String, -1, ["top", "50%", "transform", "translateY(-80%) scale(0.6)"]));
...
}}if (!this.isMenu || this.isMenuItem ) $I$(5,"setStyle",[this.menuAnchorNode, "min-width", Math.max(75, (23 + 15 + wCtr + wAccel + margins.left + margins.right )) + "px"]);
}, p$1);
